...
import { enumSexo, enumTempoVida } from '@/utils/enuns'
  ...
  const lotes = React.useMemo(() => {
    return lote.map(h => {
      ...
      const dt = {
        ...
        localizacao: `${h.distanciaMunicipio ?? '0'} km - ${h.cidade} - ${h.uf}`,
        oferta: h.qtdOfertas,
        status: h.statusLote,
        qntPeso: `${h.quantidade} | ${h.pesoMedio}`,
        tempoDeVida: vida,
        sexo: sexo as string,
        img: h.urlImagens,
        id: h.loteId
  ...
  return (
    <S.Container>
      ...
      <FlatList
        ...
        data={lotes}
        renderItem={({ item }) => <Lotes typeUser='comprador' item={item} />}
        keyExtractor={h => h.id}
        refreshControl={
          <RefreshControl
            refreshing={false}
